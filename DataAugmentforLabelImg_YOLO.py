...
# YOLO解析工具
class ToolHelper():
    # 从YOLO文件中提取bounding box信息, 格式为[[x_min, y_min, x_max, y_max, name]]
    def parse_yolo(self, txt_file_path, image_path):
        def calculate_coordinates(x_center, y_center, width, height, image_width, image_height):
        ...
        with Image.open(image_path) as img:
            ...
            for line in lines:
                ...
                if len(parts) >= 5:
                    ...
                    x_min, y_min, x_max, y_max = calculate_coordinates(
                        x_center, y_center, width, height, image_width, image_height
                    )

                    bounding_boxes.append([x_min, y_min, x_max, y_max, class_name])
    ...
    def save_yolo(self, txt_file_name, save_folder, height, width, bboxs_info):
        '''
        :param txt_file_name: 要保存的YOLO格式的文件名
        :param save_folder: 保存YOLO格式文件的目标文件夹路径
        :param img_info: 一个包含文件夹名和图片名的元组，用于构建YOLO格式文件中的图片信息
        :param width: 图片的宽度
        :param height: 图片的高度
        :param bboxs_info: 一个包含边界框标签和坐标信息的元组，用于构建YOLO格式文件中的边界框信息
        '''
        labels, bboxs = bboxs_info  # 得到边界框和标签信息

        with open(os.path.join(save_folder, txt_file_name), 'w') as txt_file:
            for label, box in zip(labels, bboxs):
                ...
                txt_file.write(f"{label} {x_center:.6f} {y_center:.6f} {bbox_width:.6f} {bbox_height:.6f}\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_img_path', type=str, default='data_orange/image_orange')
    parser.add_argument('--source_yolo_path', type=str, default='data_orange/tag_orange')
    parser.add_argument('--save_img_path', type=str, default='data_orange/image_orange2')
    parser.add_argument('--save_yolo_path', type=str, default='data_orange/tag_orange2')
    args = parser.parse_args()
    source_img_path = args.source_img_path  # 图片原始位置
    source_yolo_path = args.source_yolo_path  # yolo的原始位置

    save_img_path = args.save_img_path  # 图片增强结果保存文件
    save_yolo_path = args.save_yolo_path  # yolo增强结果保存文件

    # 如果保存文件夹不存在就创建
    if not os.path.exists(save_img_path):
        os.mkdir(save_img_path)

    if not os.path.exists(save_yolo_path):
        os.mkdir(save_yolo_path)
    ...
    for parent, _, files in os.walk(source_img_path):
        ...
        for file in files:
            ...
            txt_path = os.path.join(source_yolo_path, file[:-4] + '.txt')
            values = toolhelper.parse_yolo(txt_path, pic_path)  # 解析得到box信息，格式为[[x_min,y_min,x_max,y_max,name]]
            ...
            while cnt < need_aug_num:  # 继续增强
                ...
                toolhelper.save_yolo('{}_{}.txt'.format(_file_prefix, cnt + 1),
                                    save_yolo_path, height, width, (labels, auged_bboxes_int))  # 保存txt文件
                # show_pic(auged_img, auged_bboxes)  # 强化后的图
                print(img_name)
                cnt += 1  # 继续增强下一张

            source_file = os.path.join(source_yolo_path, 'classes.txt')
            destination_file = os.path.join(save_yolo_path, 'classes.txt')
